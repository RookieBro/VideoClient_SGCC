...
public class BitmapUtil {
    ...
    public static BitmapUtils getInstance(Context context, @DrawableRes int errImgId) {
        if (errImgId == BitmapUtil.errImgId) {
            getInstance(context);
        } else {
            bitmapUtils = new BitmapUtils(context);
            bitmapUtils.configDefaultLoadFailedImage(errImgId);
        }
        return bitmapUtils;
    }
    ...
    public static String Bitmap2StrByBase64(Bitmap bit, double width) {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bit = zoomImage(bit, width);
        bit.compress(Bitmap.CompressFormat.JPEG, 100, bos);//参数100表示不压缩
        byte[] bytes = bos.toByteArray();
        return Base64.encodeToString(bytes, Base64.DEFAULT);
    }

    public static String Bitmap2StrByBase64(Bitmap bit, int sc, Bitmap.CompressFormat f) {
        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        bit.compress(f, sc, bos);//参数100表示不压缩
        byte[] bytes = bos.toByteArray();
        return Base64.encodeToString(bytes, Base64.DEFAULT);
    }
